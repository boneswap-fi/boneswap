...
task("masterchefv2:set-a-new-rewarder", "Set farm allocation points to masterChefV2")
.addParam("pid", "Pool ID")
.addParam("alloc", "Allocation Points")
.addParam("rewarder", "Rewarder Address")
.setAction(async function ({ pid, alloc, rewarder }, { ethers: { getNamedSigner } }, runSuper) {
  const masterChefV2 = await ethers.getContract("MasterChefV2")

  // function set(uint256 _pid, uint256 _allocPoint, IRewarder _rewarder, bool overwrite) public onlyOwner {
  await (await masterChefV2.connect(await getNamedSigner("dev")).set(pid, alloc, rewarder, true, {
    gasPrice: 50000000000,
    gasLimit: 5198000,
  })).wait()
});
...
task("rewarder:pendingTokens", "Query pendingTokens of a Rewarder")
...
.setAction(async function ({ rewarder, pid, user }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  const pending = await rewarderContract.pendingTokens(pid, user, 0)
...
.setAction(async function ({ a, b }, { ethers: { getNamedSigner } }, runSuper) {
  ...
  for (let i=0; i<allPairsLength; ++i) {
    ...
    try {
      ...
      const served = await (await maker.connect(await getNamedSigner("dev")).convert(a, b, { gasPrice: 50000000000, gasLimit: 5198000 })).wait()
